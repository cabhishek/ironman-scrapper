...
module.exports = function scrape(rawHtml) {
    ...
    $('#divResults .clsResult').map(function() {
        ...
        var runTime = row.find(':nth-child(16)').text()
        var runPace = row.find(':nth-child(17)').text()
        var runAgo = row.find(':nth-child(18)').text()

        var finalTime = row.find(':nth-child(19)').text()
