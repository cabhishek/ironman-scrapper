...
module.exports = function scrapeData(data) {
      ...
      $('#divResults .clsResult').map(function(){
        ...
        var cycle_time = row.find(':nth-child(12)').text();
        var cycle_speed = row.find(':nth-child(13)').text();
        var cycle_a_g_o = row.find(':nth-child(14)').text();

        var run_time = row.find(':nth-child(15)').text();
        var run_pace = row.find(':nth-child(16)').text();
        var run_a_g_o = row.find(':nth-child(17)').text();

        var final = row.find(':nth-child(18)').text();
